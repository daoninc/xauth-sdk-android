...
class HTTP(private val params: Bundle) {
    ...
    fun post(url: String, payload: String): HttpResponse {
        Logger.logVerbose("IXUAF_KT", "HTTP post url - $url")
        try {
    ...
    fun get(url: String): HttpResponse {
        Logger.logVerbose("IXUAF_KT", "HTTP::get url $url")
        try {
            val urlConnection: HttpURLConnection = createConnection(url, GET, false)
            Logger.logVerbose("IXUAF_KT", "Server URL : ${urlConnection.url}")
            val httpResult = urlConnection.responseCode
            val response: String =
                if (
                    httpResult == HttpURLConnection.HTTP_CREATED ||
    ...
    private fun readStream(stream: InputStream): String {
        val lines = stream.use { it.bufferedReader().readLines() }
    ...
    private fun getBasicAuth(): String {
        ...
        val userAndPwd: String = username + ":" + password
    ...
    private fun getAbsoluteUrl(entity: String): String {
        ...
        // return scheme + "://" + url.getHost() + ":" + port + url.getPath() + "/" + RESTPath + "/"
        // + entity
    }
